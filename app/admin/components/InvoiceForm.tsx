...
type Client = { id: string; companyName: string; firstName: string; lastName: string };
...
export function InvoiceForm({ clients }: { clients: Client[] }) {
  ...
  const totalTTC = totalHT * 1.20; // Assumes 20% TVA

  return (
    <form action={createInvoice} className="nv-card" style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
        ...
        <div>
          <label className="nv-label">Date d'échéance</label>
          <input type="date" name="dueDate" className="nv-input" required />
        </div>
      </div>
